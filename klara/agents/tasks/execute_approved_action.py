...
───────────────────────────
  ...
  translation_agent.publish  → pushes translated content to WordPress as draft
...
async def _execute(approval_id: str):
    ...
    async with db_context() as db:
        ...
        elif action == "translation_agent.publish":
            await _run_translation_publish(db, settings, payload, req)
...
async def _run_translation_publish(db, settings, payload: dict, req: ApprovalRequest):
    """
    Publish an approved translation to WordPress as a draft via WebsiteDeployAgent.

    Two modes (controlled by payload["mode"]):
      update_page  — PATCH an existing WP page (target_page_id is required).
                     Uses action="execute" with a synthesised WebsiteDeployJob.
      create_post  — POST a new WP draft post/page.
                     Uses action="create_post" directly.

    Expected payload keys (set by TranslationAgent when it queues approval):
      source_page_id  — source WP page/post ID (int)
      target_page_id  — target WP page/post ID (int | None)
      source_language — "en" | "de"
      target_language — "de" | "en"
      source_title    — original page title (str)
      title           — translated page title (str)
      content_html    — translated HTML content (str)
      post_type       — "page" | "post"
    ...
    mode          = payload.get("mode", "create_post")
    title         = payload.get("title", "")
    content_html  = payload.get("content_html", "")
    ...
    if mode == "update_page" and target_page_id:
        ...
        job = WebsiteDeployJob(
            id=_uuid.uuid4(),
            page_id=int(target_page_id),
            page_title=title,
            new_content=content_html,
            status=DeployJobStatus.approved,
            queued_by="translation_agent",
        )
        db.add(job)
    ...
    result = await agent(context, deploy_payload)
    logger.info(
        "execute_approved_action.translation_publish_complete",
        mode=mode,
...
async def _run_invoice_send(db, settings, payload: dict, req: ApprovalRequest):
    ...
    body_html = textwrap.dedent(f"""\
        ...
        <p style="color:#888;margin-top:0;">klaravex.de</p>
        ...
        <p>Payment by bank transfer (SEPA) — details are on the invoice PDF.</p>
        ...
        Klaravex<br>
        <a href="https://klaravex.de">klaravex.de</a></p>

        <hr style="border:none;border-top:1px solid #e0e0e0;margin:16px 0;">
        <p style="font-size:11px;color:#aaa;">
          Klaravex · Berlin, Germany · hello@klaravex.de
        </p>
        </body>
        </html>
    """)
    ...
    body_text = (
        ...
        f"Payment by bank transfer (SEPA) — details are on the invoice PDF.\n\n"
