...
──────────────────────────────────────────────────────
  ...
  generate-eu-social-drafts        Mon–Fri 09:00 Berlin — EU/DACH clients  ← LIVE
...
─────────────────────
  ...
  generate_weekly_social_drafts     — LIVE: called by both EU and US n8n triggers with market kwarg
  publish_scheduled_posts           — pending platform API integration
  collect_daily_analytics_task      — pending platform API integration
  generate_weekly_digest            — pending platform API integration
"""
...
@celery_app.task(
    name="app.tasks.social_media.generate_weekly_social_drafts",
    bind=True,
    max_retries=2,
    default_retry_delay=300,
)
def generate_weekly_social_drafts(self: Any, market: str = "eu") -> dict[str, Any]:
    """
    Triggered twice daily Mon–Fri by n8n:
      - 09:00 Berlin with market="eu" (EU/DACH clients, includes XING)
      - 15:00 Berlin with market="us" (US/NA clients, LinkedIn/Twitter/Facebook/Reddit)

    For each run:
      1. Generate platform-specific topics via Claude (market-aware)
...
async def _generate_weekly_drafts(market: str = "eu") -> dict[str, Any]:
    ...
    region = "United States / North America" if market == "us" else "Germany / EU"
...
def publish_scheduled_posts(self: Any) -> dict[str, Any]:
    ...
    No-op until LinkedIn / XING / X / Facebook API integration is complete.
    Approved posts are currently surfaced to Anthony via the admin dashboard.
    """
    logger.info(
        "social_media.publish_scheduled_posts.noop",
