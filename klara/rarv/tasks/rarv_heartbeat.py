...
_GH_COMMITTER = {"name": "RARV Journal Team", "email": "rarv-journal@klaravex.com"}
...
async def _process_one(sub_id: int) -> str:
    ...
    async with db_context() as db:
        ...
        concerns = (reflector_res.output or {}).get("concerns") or []
        if concerns:
            logger.warning(
                "rarv_heartbeat.reflector_concerns",
                submission_id=sub_id,
                concerns=concerns,
            )
...
async def _write_to_github(
    *,
    ...
    """
    Create or update a file in the klaravex-vault GitHub repo via the Contents API.
