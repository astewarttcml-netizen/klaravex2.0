    ...
    try:
        ...
        try:
            ...
            _asyncio.run(pipeline_alert(
                "leads", "task_failed", "critical",
                f"Prospecting task crashed: {exc}"
            ))
        except Exception:
            pass
...
async def _prospect_and_queue(triggered_by: str) -> dict:
    ...
    async with db_context() as db:
        ...
        if not new_prospects:
            ...
            if getattr(settings, "apollo_configured", False):
                try:
                    from app.services.pipeline_alert import pipeline_alert
                    await pipeline_alert(
                        "leads", "no_new_prospects", "warning",
                        "Prospecting returned 0 results — possible Apollo API issue"
                    )
                except Exception:
                    pass
        ...
        # ── 2. Dispatch research + outreach for each prospect ──────────────
        # Each prospect goes through run_research (web scraping + local LLM
        # enrichment) which then chains to prospecting_outreach with a
        # research_bundle for personalized emails. This replaces the old
        # direct-to-outreach path that produced generic template emails.
        from app.tasks.research_prospect import run_research

        for prospect in new_prospects:
            try:
                run_research.delay(str(prospect.id))
                queued_count += 1
                logger.info(
                    "prospect_leads.queued_research",
                ...
                failed_count += 1
                logger.error(
                    "prospect_leads.research_dispatch_failed",
                    domain=prospect.domain,
                    error=str(exc),
                    exc_info=True,
                )
    ...
    # Daily summary + high failure rate warning
    try:
        from app.services.pipeline_alert import pipeline_alert
        await pipeline_alert(
            "leads", "daily_summary", "info",
            f"Prospecting: {len(new_prospects)} found, "
            f"{queued_count} queued, {failed_count} failed"
        )
        if failed_count >= 3:
            await pipeline_alert(
                "leads", "high_failure_rate", "warning",
                f"High draft failure rate: {failed_count}/{len(new_prospects)}"
            )
    except Exception:
        pass
