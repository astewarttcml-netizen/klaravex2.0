...
from app.services.prospecting_send import dispatch_via_smartlead
...
def _pick_language(contact_email: str | None) -> str:
    """US-only override (2026-07-19, Anthony directive): always English while
    this environment serves the US market only. Original .de-TLD detection
    kept below, commented out, for when DACH/EU work resumes — do not
    delete, do not re-enable without explicit direction."""
...
async def _run() -> dict:
    ...
    async with db_context() as db:
        ...
        for seq, _approval, prospect in rows:
            ...
            # Routed through Smartlead (2026-07-19) — same warmed-mailbox
            # transport as the initial outreach send. Follow-ups previously
            # went out via send_transactional_email (Resend/SMTP direct from
            # the main klaravex.com domain), which risked burning that
            # domain's sender reputation instead of Smartlead's dedicated,
            # warmed sending infrastructure. dispatch_via_smartlead adds this
            # prospect to their industry-routed campaign the same way the
            # initial send does; Smartlead itself then handles delivery.
            ok = await dispatch_via_smartlead(
                settings=settings,
                prospect=prospect,
                subject=subject,
