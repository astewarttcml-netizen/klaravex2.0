        ...
        async with db_context() as db:
            ...
            if result.success:
                ...
                try:
                    ...
                    await pipeline_alert("freelancer", "scan_failed", "warning",
                                         f"Platform scan failed: {result.error}")
                except Exception:
                    pass
    ...
    async def _run():
        ...
        async with db_context() as db:
            ...
            if result.success:
                ...
                try:
                    from app.services.pipeline_alert import pipeline_alert
                    await pipeline_alert("freelancer", "bid_strategy_failed", "warning",
                                         f"Bid strategy failed: {result.error}")
                except Exception:
                    pass
            return result.output
    ...
    async def _run():
        ...
        async with db_context() as db:
            ...
            if result.success:
                ...
                # Alert if there were submission errors
                errs = result.output.get("errors", 0)
                if errs > 0:
                    try:
                        from app.services.pipeline_alert import pipeline_alert
                        await pipeline_alert("freelancer", "bid_submission_errors", "warning",
                                             f"Bid submission completed with {errs} errors — "
                                             f"submitted={result.output.get('submitted',0)}, "
                                             f"manual={result.output.get('manual_required',0)}")
                    except Exception:
                        pass
            else:
                logger.error(
                ...
                try:
                    from app.services.pipeline_alert import pipeline_alert
                    await pipeline_alert("freelancer", "bid_submission_failed", "warning",
                                         f"Bid submission failed: {result.error}")
                except Exception:
                    pass
            return result.output
            ...
            try:
                ...
                await send_transactional_email(
                    ...
                    body_html=(
                        ...
                        "<code>~/klaravex/infra/docker-services/worker/.env</code> and run "
                    ...
                    body_text=(
                        ...
                        "~/klaravex/infra/docker-services/worker/.env (FREELANCERMAP_SESSION_COOKIE), then:\n"
        ...
        async with db_context() as db:
            ...
            try:
                ...
                await pipeline_alert(
                    "freelancer", "daily_stats", "info",
                    f"Pipeline stats:\n"
                    f"• Bids: {bid_stats}\n"
                    f"• Projects: {proj_stats}"
                )
            except Exception:
                pass
