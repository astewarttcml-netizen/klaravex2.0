...
- Spelling/grammar/clarity issues in the body (soft warning, LLM-checked
  via Anthropic; fails open — an LLM error never blocks or fails the
  submission, it just skips this one check)
...
_SPELLING_CHECK_MIN_CHARS = 40  # skip trivially short bodies, not worth a call
_SPELLING_CHECK_MAX_CHARS = 20_000  # skip huge bodies, not what this check is for
...
class RARVReflectorAgent(BaseAgent):
    ...
    async def run(self, context: AgentContext, input_data: dict) -> AgentResult:
        ...
        spelling_concern = await self._spelling_and_clarity_check(context, body_md)
        if spelling_concern:
            concerns.append(spelling_concern)
    ...
    @staticmethod
    async def _spelling_and_clarity_check(context: AgentContext, body_md: str) -> str | None:
        """
        Soft check only -- never blocks, never rejects. Returns a single
        concern string summarizing genuine spelling/grammar/clarity issues,
        or None if there's nothing worth flagging (including on any error --
        this check fails open, same as the rest of the pipeline's IO calls).
        """
        text = body_md.strip()
        if not (_SPELLING_CHECK_MIN_CHARS <= len(text) <= _SPELLING_CHECK_MAX_CHARS):
            return None
        ...
        try:
                ...
                messages=[
                    {
                        "role": "user",
                        "content": (
                            "Proofread this note for genuine spelling errors, grammar "
                            "mistakes, and unclear/ambiguous sentences only -- NOT style "
                            "preferences, NOT technical jargon, NOT code/config syntax, "
                            "NOT intentional informal tone. This is an internal ops log, "
                            "not published prose -- be conservative, most notes should "
                            "have zero issues.\n\n"
                            "Respond with ONLY a JSON object, no other text:\n"
                            '{"issues": ["short description", ...]} -- empty list if none.\n\n'
                            f"---\n{text}\n---"
                        ),
                    }
                ],
            ...
            # Tolerate the model wrapping JSON in a code fence despite instructions.
            if raw.startswith("```"):
            ...
            if not issues:
                return None
            return "spelling/clarity: " + "; ".join(issues[:5])
        except Exception as e:
            logger.warning("reflector.spelling_check.failed", error=str(e))
            return None
