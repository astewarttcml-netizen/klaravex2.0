...
DISABLED_TRIGGERS: set[str] = {
    # Social draft generation owned exclusively by celery_app.py's own
    # beat_schedule now (2026-07-19, Anthony directive: exactly 2x/day, one
    # timed for ET and one for PT — Klaravex is US-only). n8n's external cron
    # for these two is out of this repo's control and was firing on its own
    # unpredictable cadence (every ~2 days per the TTL below, actual n8n
    # schedule unknown) — gating both off here makes celery_app.py the single
    # source of truth instead of two independent schedulers racing.
    "generate-us-social-drafts",
...
TASK_REGISTRY: dict[str, tuple[str, int, dict[str, Any]]] = {
    ...
    # Kept registered so n8n's existing POSTs get a clean 200 "skipped"
    # instead of a 404. Gated off via DISABLED_TRIGGERS (2026-07-19) —
    # social draft generation is owned by celery_app.py's own beat_schedule.
    "generate-us-social-drafts": (
...
def _get_klaravex_broker_url() -> str:
    ...
    klaravex_worker container uses host.docker.internal:6379/1).
...
async def trigger_task(
    ...
    # ── Idempotency check ────────────────────────────────────────────────────
    # Fail-open: if Redis is unreachable (e.g. Azure Container App can't reach
    # the Hetzner Docker-network Redis at redis://redis:6379), log a warning
    # and proceed without idempotency protection. The n8n schedule cadence
    # already prevents over-firing in practice.
    try:
        r = await _get_redis()
        last_run: str | None = await r.get(redis_key)
    except Exception as exc:
        log.warning("beat_trigger.redis_unavailable", task_name=task_name, error=str(exc))
        r = None
        last_run = None
    ...
    try:
        ...
        return TriggerResponse(
            ...
            task_name=task_name,
            celery_task=celery_task_path,
            skipped_reason=f"Celery dispatch failed: {exc}",
            last_triggered_at=None,
        )
    ...
    if r is not None:
        try:
            ...
            log.warning("beat_trigger.redis_set_failed", task_name=task_name, error=str(exc))
